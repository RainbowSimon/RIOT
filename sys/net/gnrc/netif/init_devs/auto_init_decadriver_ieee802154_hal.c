...
void auto_init_decadriver_hal(void)
{
    LOG_DEBUG("[auto_init_netif] initializing decadriver radio\n");

    netdev_register(&cc2538_rf_netdev.dev.netdev, NETDEV_DECADRIVER, 0);
    netdev_ieee802154_submac_init(&cc2538_rf_netdev);
    dw3000_ieee802154_hal_setup(&cc2538_rf_netdev.submac.dev);

    dw3000_ieee802154_init();
    gnrc_netif_ieee802154_create(&_netif, _cc2538_rf_stack,
                                 CC2538_MAC_STACKSIZE,
                                 CC2538_MAC_PRIO, "decadriver",
